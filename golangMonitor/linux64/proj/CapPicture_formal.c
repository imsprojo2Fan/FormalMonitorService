...
**********************************************************************/
long AutoSnap(char *sPicFileName,int channelNum){
    long errCode;
    NET_DVR_Init();
    long lUserID;
    //login
    NET_DVR_DEVICEINFO_V30 struDeviceInfo;
    lUserID = NET_DVR_Login_V30("120.27.102.239", 33333, "admin", "a1234567", &struDeviceInfo);   
...
long AutoDownload(char *sVideoFileName,int channelNum,long pYear,int pMonth,int pDay,int pHour_start,int pMinute_start,int pSecond_start,int pHour_end,int pMinute_end,int pSecond_end) {
        ...
        struDownloadCond.dwChannel= struDeviceInfo.byStartChan; //1; 
        ...
        struDownloadCond.struStartTime.dwDay =12;
        struDownloadCond.struStartTime.dwHour= 14;
        struDownloadCond.struStartTime.dwMinute =20;
        ...
        struDownloadCond.struStopTime.dwDay = 12;
        struDownloadCond.struStopTime.dwHour = 14;
        struDownloadCond.struStopTime.dwMinute = 30;
        struDownloadCond.struStopTime.dwSecond =6;*/
        /*time_t tmpcal_ptr;  
        long tmpcal_ptr_start;
        struct tm *tmp_ptr = NULL;  
        struct tm *tmp_ptr_start = NULL; 
        time(&tmpcal_ptr);
        tmpcal_ptr_start = tmpcal_ptr-3000;
        printf("start=%d\n", tmpcal_ptr_start); 
        printf("end=%d\n", tmpcal_ptr); 
        tmp_ptr = localtime(&tmpcal_ptr);
        tmp_ptr_start = localtime(&tmpcal_ptr_start);
        struDownloadCond.struStartTime.dwYear = (1900+tmp_ptr_start->tm_year);
        struDownloadCond.struStartTime.dwMonth = (1+tmp_ptr_start->tm_mon);
        struDownloadCond.struStartTime.dwDay = tmp_ptr_start->tm_mday;
        struDownloadCond.struStartTime.dwHour= tmp_ptr_start->tm_hour;
        struDownloadCond.struStartTime.dwMinute = tmp_ptr_start->tm_min;
        struDownloadCond.struStartTime.dwSecond = tmp_ptr_start->tm_sec;
        struDownloadCond.struStopTime.dwYear = (1900+tmp_ptr->tm_year);
        struDownloadCond.struStopTime.dwMonth = (1+tmp_ptr->tm_mon);
        struDownloadCond.struStopTime.dwDay = tmp_ptr->tm_mday;
        struDownloadCond.struStopTime.dwHour = tmp_ptr->tm_hour;
        struDownloadCond.struStopTime.dwMinute = tmp_ptr->tm_min;
        struDownloadCond.struStopTime.dwSecond = tmp_ptr->tm_sec;*/
...
long Test() {
        ...
        struDownloadCond.struStartTime.dwHour= 16;
        struDownloadCond.struStartTime.dwMinute =10;
        struDownloadCond.struStartTime.dwSecond =0;
        struDownloadCond.struStopTime.dwYear =2018;
        struDownloadCond.struStopTime.dwMonth = 6;
        struDownloadCond.struStopTime.dwDay = 11;
        struDownloadCond.struStopTime.dwHour = 16;
...
long DownloadByTime(char *sPicFileName,int channelNum,long pYear,int pMonth,int pDay,int pHour_start,int pMinute_start,int pSecond_start,long endYear,int endMonth,int endDay,int pHour_end,int pMinute_end,int pSecond_end) {
        ...
        NET_DVR_DEVICEINFO_V30 struDeviceInfo;
        lUserID = NET_DVR_Login_V30("120.27.102.239", 33333, "admin", "a1234567", &struDeviceInfo); 
        //lUserID = NET_DVR_Login_V30("192.168.88.8", 8000, "admin", "a1234567", &struDeviceInfo);
        if (lUserID < 0)
        {
        printf("Login error, %d\n", NET_DVR_GetLastError()); 
        errCode = NET_DVR_GetLastError();
